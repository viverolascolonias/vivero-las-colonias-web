...
// Colores agrupados por variante para que un `className` externo nunca tenga
// que pisar bg-/text- por separado (con Tailwind eso es frágil: la clase que
// "gana" depende del orden en que Tailwind generó el CSS, no del orden en el
// string, así que mezclar override de color por className puede dejar texto
// oscuro sobre fondo oscuro sin que se note en el código).
const VARIANT_CLASSES = {
  dark: "bg-[var(--color-forest)] text-[var(--color-cream-50)] hover:bg-[var(--color-forest-dark)]",
...
export default function WhatsAppButton({
  ...
  return (
    <a
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-medium tracking-wide transition-colors duration-200 ${VARIANT_CLASSES[variant]} ${className}`}
    >
